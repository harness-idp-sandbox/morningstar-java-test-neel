...
A sample Java application built with Spring Boot test
...
# run app
java -jar target/java-test-neel-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-test-neel:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/java-test-neel -n dev

# get external endpoint
kubectl get svc java-test-neel-svc -n dev
```
...
```bash
kubectl port-forward svc/java-test-neel-svc 8080:80 -n dev
...
kubectl rollout restart deploy/java-test-neel -n dev
kubectl rollout status deploy/java-test-neel -n dev
```
...
info.app.name=javatestneel
info.app.description=A sample Java application built with Spring Boot test
```
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javatestneel/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
